...
func _update_sprite_direction() -> void:
	if animated_sprite and animated_sprite.flip_h == GlobalStates.facing_right:
		...
		character.interaction_ray_cast.target_position = Vector2(50,0) if GlobalStates.facing_right else Vector2(-50,0)
		character.wall_ray_cast.target_position = Vector2(7,0) if GlobalStates.facing_right else Vector2(-7,0)
